...
The consensus matrix at the chosen rank should be a crisp block-diagonal:

```{r consensus-heatmap, fig.width = 6.5, fig.height = 6.5}
plot(cs, type = "heatmap", rank = 3)   # blue = 0 (different), red = 1 (same)
```
...
On this clean dataset with a clear rank-3 structure, the predictive
cross-validations (`nmfkc.ecv`, `nmfkc.bicv`) and the integrated `nmfkc.rank`
recover the true rank exactly (`3`), and the consensus `dispersion` also peaks
at `3` with a crisp three-block consensus matrix.  Notably, the two
stability/model criteria reveal their **known tendencies even here**: `PAC`
leans to the coarser rank `2`, and the ARD point estimate lands slightly high
(`4`).  This already previews the behaviour on harder data.
...
when the energy spectrum decays smoothly.  A robust practical recipe is to
**triangulate**:
