## Tests for nmf.cluster() sample-clustering diagnostics

test_that("nmf.cluster computes silhouette/CPCC/dist.cor for non-negative B", {
  Y <- t(as.matrix(iris[, 1:4]))
  fit <- nmfkc(Y, Q = 3, print.dims = FALSE)
  cl <- nmf.cluster(fit, Y)

  expect_s3_class(cl, "nmf.cluster")
  expect_true(cl$hard)
...
test_that("nmf.cluster sets silhouette NA when the coefficient is signed", {
  set.seed(7)
...
test_that("nmf.cluster matches nmfkc.criterion's values (shared helper)", {
  Y <- t(as.matrix(iris[, 1:4]))
  fit <- nmfkc(Y, Q = 3, print.dims = FALSE)
  cl  <- nmf.cluster(fit, Y)
  crit <- fit$criterion
...
test_that("nmf.cluster requires Y", {
  Y <- t(as.matrix(iris[, 1:4]))
  fit <- nmfkc(Y, Q = 2, print.dims = FALSE)
  expect_error(nmf.cluster(fit), "requires the original data")
})
