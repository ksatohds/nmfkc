...
| `nmf.ffb()` / `nmf.ffb.inference()` | NMF Feed-Forward + Feedback model (formerly `nmf.sem*`, retained as alias): basis from `nmfkc()`, then FIML with an exclusion mask, L1 path and BIC selection of the feedback (`method = "fiml"`, default; legacy `method = "mu"`) + parametric-bootstrap calibration of the LR test and coefficient intervals |
